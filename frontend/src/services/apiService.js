...
export const authAPI = {
  checkPhone: (phoneNumber) =>
    api.post("/auth/check-phone", { phone_number: phoneNumber }),
  ...
  loginWithOTP: (phoneNumber) =>
    api.post("/auth/login-with-otp", { phone_number: phoneNumber }),
...
export const bankAPI = {
  ...
  setUpiPin: (accountId, pin) =>
    api.post(`/bank/accounts/${accountId}/pin`, { pin }),
  verifyPin: (accountId, pin) =>
    api.post("/bank/verify-pin", { accountId, pin }),
  deposit: (accountId, amount) =>
    api.post("/bank/deposit", { accountId, amount }),
  withdraw: (accountId, amount) =>
    api.post("/bank/withdraw", { accountId, amount }),
  transfer: (fromAccountId, toAccountId, amount, pin) =>
    api.post("/bank/transfer", { fromAccountId, toAccountId, amount, pin }),
  createVerificationOrder: () => api.post("/bank/verification-order"),
  createVerifiedAccount: (payload) =>
    api.post("/bank/verified-accounts", payload),
};
...
export const reserveAPI = {
  ...
  checkPayment: (merchant, amount) =>
    api.post("/reserve/check-payment", { merchant, amount }),
  processPayment: (merchant, amount, transactionId, description) =>
    api.post("/reserve/process-payment", {
      merchant,
      amount,
      transactionId,
      description,
...
export const coinAPI = {
  ...
  redeem: (coinAmount, type, target) =>
    api.post("/coins/redeem", { coinAmount, type, target }),
};
...
export const moneyRequestAPI = {
  getAll: (status = null) =>
    api.get(`/money-requests${status ? `?status=${status}` : ""}`),
  create: (requestData) => api.post("/money-requests", requestData),
  updateStatus: (requestId, status) =>
    api.put(`/money-requests/${requestId}/status`, { status }),
};
...
export const agentOrderAPI = {
  ...
  selectItems: (sessionId, selectedItems, selection) =>
    api.post("/agent/order/select-items", {
      sessionId,
      selectedItems,
      selection,
    }),
  ...
  checkMerchantConnection: (merchantId) =>
    api.get(`/agent/order/check-connection/${merchantId}`),

  // Auto-Pay
  setupAutoPay: (sessionId, schedule, dayOfMonth, bankAccountId, orderData) =>
    api.post("/agent/order/auto-pay/setup", {
      sessionId,
      schedule,
      dayOfMonth,
      bankAccountId,
  ...
  syncReserveLimits: (limits) =>
    api.post("/agent/order/sync-reserve-limits", { limits }),
  ...
  saveTransaction: (transaction) =>
    api.post("/agent/order/save-transaction", transaction),
...
export const splitRequestAPI = {
  ...
  updateStatus: (splitId, status) =>
    api.put(`/split-requests/${splitId}/status`, { status }),
...
export const merchantAPI = {
  ...
  disconnectMerchant: (merchantId) =>
    api.post("/merchant/disconnect", { merchantId }),
  updateLastUsed: (merchantId) =>
    api.post("/merchant/update-last-used", { merchantId }),
  ...
  saveLocation: (merchantId, address, city, area, coordinates) =>
    api.post("/merchant/save-location", {
      merchantId,
      address,
      city,
      area,
  ...
  deleteLocation: (merchantId) =>
    api.delete(`/merchant/location/${merchantId}`),

  getRestaurants: (merchantId, city, area) =>
    api.get(
      `/merchant/${merchantId}/restaurants?city=${city}&area=${area || ""}`,
    ),

  getRestaurantMenu: (merchantId, restaurantId, city) =>
    api.get(
      `/merchant/${merchantId}/restaurant/${restaurantId}/menu?city=${city}`,
    ),

  searchItems: (merchantId, city, searchTerm, area) =>
    api.get(
      `/merchant/${merchantId}/search?city=${city}&q=${searchTerm}&area=${area || ""}`,
    ),

  getSuggestions: (merchantId, city, area, limit = 5) =>
    api.get(
      `/merchant/${merchantId}/suggestions?city=${city}&area=${area || ""}&limit=${limit}`,
    ),

  checkCity: (merchantId, city) =>
    api.get(`/merchant/${merchantId}/city/${city}/check`),
};

export const agentPaymentAPI = {
  // Send Money
  resolveRecipient: (text) => {
    return api.post("/agent/payment/send-money/resolve-recipient", { text });
  },
  sendMoney: (recipient, amount, note) => {
    return api.post("/agent/payment/send-money", { recipient, amount, note });
  },
  confirmSendMoney: (paymentData, pin) => {
    return api.post("/agent/payment/send-money/confirm", { paymentData, pin });
  },

  // Request Money
  requestMoney: (recipient, amount, note) => {
    return api.post("/agent/payment/request-money", {
      recipient,
      amount,
      note,
    });
  ...
  listBillers: (category) => {
    return api.get("/agent/payment/bills/list-billers", {
      params: { category },
    });
  },
  fetchBillAmount: (billerId, customerId) => {
    return api.post("/agent/payment/bills/fetch-amount", {
      billerId,
      customerId,
    });
  },
  payBill: (billType, provider, customerId, amount) => {
    return api.post("/agent/payment/bills/pay", {
      billType,
      provider,
      customerId,
      amount,
  ...
  confirmBillPayment: (paymentData, pin) => {
    return api.post("/agent/payment/bills/confirm", { paymentData, pin });
  },

  // Recharge
  detectOperator: (mobileNumber) => {
    return api.post("/agent/payment/recharge/detect-operator", {
      mobileNumber,
    });
  },
  listRechargePlans: (operatorId) => {
    return api.post("/agent/payment/recharge/list-plans", { operatorId });
  },
  rechargeMobile: (mobileNumber, amount, plan) => {
    return api.post("/agent/payment/recharge/pay", {
      mobileNumber,
      amount,
      plan,
    });
  ...
  confirmRecharge: (paymentData, pin) => {
    return api.post("/agent/payment/recharge/confirm", { paymentData, pin });
  },

  // Multi Payment
  multiPayment: (payments) => {
    return api.post("/agent/payment/multi-payment/build", { payments });
  },
  confirmMultiPayment: (payments, pin) => {
    return api.post("/agent/payment/multi-payment/confirm", { payments, pin });
  },

  // Security
  checkSabAIPayLite: (amount) => {
    return api.post("/agent/payment/security/check-limit", { amount });
  },
  getSabAIPayLite: () => {
    return api.get("/agent/payment/security/sabai-pay-lite");
  },
};
