    ...
    >
      <div className="popupi-content">
        ...
        <motion.div
          ...
          animate={{
            height: animationStage >= 3 ? "auto" : 0,
          ...
          >
            ...
            <div className="detail-item highlight failed">
              ...
              <span style={{ color: "#ef4444" }}>
                {transactionData?.failure_reason || "Insufficient balance"}
              </span>
            </div>
            <div className="detail-item">
              <span>Date & Time</span>
...
// ============================================
// CUSTOM PAYMENT MODAL (Existing - Keep as is)
// ============================================
// [CustomPaymentModal component - unchanged from your original]
// I'm including it here but it's the same as your existing one
    ...
    if (logoUrl && !hasError) {
      return (
        <img
          ...
          onError={() =>
            setImageErrors((prev) => ({ ...prev, [`bank_${bank.id}`]: true }))
          }
        />
      );
    }
          ...
          {payStep === 1 && (
            <>
                ...
                <div className="gems-input-section">
                  ...
                  <div className="gems-input-wrapper">
                    <input
                      ...
                      onChange={(e) =>
                        handleGemsAmountChange(parseInt(e.target.value) || 0)
                      }
                      min="0"
                  ...
                  <p className="gems-balance">
                    Available: {sabaiGems} 🪙 (1 🪙 = ₹1)
                  </p>
                  {paymentBreakdown.gemsAmount > 0 && (
                    <div className="payment-breakdown-preview">
                      <p>
                        After using {paymentBreakdown.gemsAmount} 🪙:{" "}
                        <strong>
                          ₹
                          {paymentBreakdown.remainingAfterGems.toLocaleString()}
                        </strong>{" "}
                        remaining
                      </p>
                    </div>
                  ...
                  {paymentBreakdown.gemsAmount === 0 && (
                    <span className="cashback-info">
                      You'll earn +{calculateCashback(orderData.total)} 🪙
                      cashback
                    </span>
                  )}
                  {paymentBreakdown.gemsAmount > 0 && (
                    <span className="cashback-info no-cashback">
                      ⚠️ No cashback when using Gems
                    </span>
                  )}
                </div>
              )}
              ...
              <div className="payment-methods-section">
                ...
                {sabaiGems >= orderData.total && (
                  ...
                  >
                    ...
                    <div className="payment-method-info">
                      ...
                      <span>
                        Use {orderData.total.toLocaleString()} 🪙 (No cashback)
                      </span>
                    </div>
                    {selectedPaymentType === "gems_only" && (
                ...
                {availableReserveLimit >= orderData.total && (
                  ...
                  >
                    ...
                    <div className="payment-method-info">
                      ...
                      <span>
                        Use ₹{orderData.total.toLocaleString()} from your
                        universal limit
                      </span>
                      <span className="limit-info">
                        Available: ₹{availableReserveLimit.toLocaleString()}
                      </span>
                    </div>
                    {selectedPaymentType === "reserve_pay" && (
                      <FaCheckCircle className="selected-icon" />
                    )}
                  </button>
                )}
                ...
                {paymentBreakdown.gemsAmount > 0 &&
                  paymentBreakdown.remainingAfterGems > 0 &&
                  availableReserveLimit >=
                    ...
                    >
                      <div className="payment-method-icon gems-lite">
                        <img
                          src="/images/sabaigems.png"
                          alt="Gems"
                          className="payment-method-logo-small"
                        />
                        <img
                          src="/images/merchants/sabailogo.png"
                          alt="SabAI"
                      ...
                      <div className="payment-method-info">
                        ...
                        <span>
                          Use {paymentBreakdown.gemsAmount} 🪙 + ₹
                          {paymentBreakdown.remainingAfterGems.toLocaleString()}{" "}
                          from limit
                        </span>
                      </div>
                      {selectedPaymentType === "gems_and_lite" && (
                        <FaCheckCircle className="selected-icon" />
                      )}
                    </button>
                  )}
                      ...
                      <div className="banks-list-pay">
                        {banksWithPin.map((bank) => (
                          <button
                            ...
                            onClick={() =>
                              handlePaymentMethodSelect("bank", bank)
                            }
                          >
                            ...
                            <div className="bank-info-pay">
                              <span className="bank-name-pay">
                                {bank.bank_name}
                              </span>
                              <span className="bank-account-pay">
                                xxxx{bank.account_number?.slice(-4)}
                              </span>
                            </div>
                            {selectedPaymentMethod?.id === bank.id &&
                              selectedPaymentType === "bank" && (
                                <FaCheckCircle className="selected-icon-pay" />
                              )}
                          </button>
                        ))}
                      </div>
                ...
                {paymentBreakdown.gemsAmount > 0 &&
                  paymentBreakdown.remainingAfterGems > 0 && (
                    <div className="combined-payment-info">
                      <FaInfoCircle />
                      <span>
                        You'll pay {paymentBreakdown.gemsAmount} 🪙 + ₹
                        {paymentBreakdown.remainingAfterGems.toLocaleString()}{" "}
                        via selected method
                      </span>
                    </div>
              ...
              <div className="modal-footer">
                ...
                <button
                  ...
                  onClick={() => {
                    ...
                    if (
                      (selectedPaymentType === "bank" ||
                        selectedPaymentType === "gems_and_bank") &&
            ...
            <>
              <div className="payment-summary">
                ...
                <div className="summary-row">
                  <span>From</span>
                  <span>
                    {selectedPaymentMethod.bank_name} (xxxx
                ...
                {paymentBreakdown.bankAmount > 0 && (
                  <div className="summary-row">
                    ...
                    <strong>
                      ₹{paymentBreakdown.bankAmount.toLocaleString()}
                    </strong>
                  </div>
                )}
                {paymentBreakdown.reserveAmount > 0 && (
                  <div className="summary-row">
                    ...
                    <strong>
                      ₹{paymentBreakdown.reserveAmount.toLocaleString()}
                    </strong>
                  </div>
                )}
                {paymentBreakdown.gemsAmount === 0 &&
                  ...
                  paymentBreakdown.bankAmount > 0 && (
                    <div className="summary-row">
                      ...
                      <span className="cashback-amount">
                        +{calculateCashback(orderData.total)} 🪙
                      </span>
                    </div>
                  )}
                {(paymentBreakdown.gemsAmount > 0 ||
                  ...
                  <div className="summary-row">
                    <span>Cashback</span>
                    <span className="cashback-amount no-cashback">
                      0 🪙 (Gems/Reserve Pay used)
                    </span>
                  </div>
                )}
              </div>
              ...
              <div className="pin-section-pay">
                <label>
                  Enter UPI PIN for {selectedPaymentMethod.bank_name}
                </label>
                <div className="pin-inputs-pay">
                  {payPinDigits.map((digit, index) => (
                    <input
                      ...
                      value={digit}
                      onChange={(e) =>
                        handlePayPinChange(index, e.target.value)
                      }
                ...
                <label className="show-pin-checkbox-pay">
                  <input
                    type="checkbox"
                    checked={showPayPin}
                    onChange={() => setShowPayPin(!showPayPin)}
                  />
                  <span>Show PIN</span>
                </label>
              ...
              <div className="modal-footer">
                ...
                >
                  {payLoading ? (
                    ...
                    `Pay ₹${paymentBreakdown.bankAmount.toLocaleString()}`
                  )}
                </button>
              </div>
            </>
                  ...
                  <div className="payment-summary">
                    ...
                    {pendingPaymentData.paymentBreakdown.gemsAmount > 0 && (
                      <div className="summary-row">
                        <span>SabAI Gems Used</span>
                        <span>
                          {pendingPaymentData.paymentBreakdown.gemsAmount} 🪙 (₹
                          {pendingPaymentData.paymentBreakdown.gemsAmount})
                        </span>
                      </div>
                    )}
                    {pendingPaymentData.paymentBreakdown.reserveAmount > 0 && (
                      <div className="summary-row">
                        ...
                        <span>
                          ₹
                          {pendingPaymentData.paymentBreakdown.reserveAmount.toLocaleString()}
                        </span>
                      </div>
                    )}
                    {pendingPaymentData.paymentBreakdown.bankAmount > 0 && (
                      <div className="summary-row">
                        ...
                        <span>
                          ₹
                          {pendingPaymentData.paymentBreakdown.bankAmount.toLocaleString()}
                        </span>
                      </div>
                    )}
                    <div className="summary-row">
        ...
        <div className="modal-body">
          ...
          <div className="form-group">
            ...
            <p className="field-note">
              Payment will be made on this day every month
            </p>
          </div>
                ...
                bankAccounts.map((bank) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {!hasPin && (
                        <span className="pin-warning">🔒 PIN not set</span>
                      )}
          ...
          <div className="auto-pay-info-note">
            <FaInfoCircle />
            <span>
              Amount will be automatically deducted from your selected bank
              account on the scheduled date. Ensure sufficient balance.
            </span>
          </div>
        </div>
...
const AgentChatPage = () => {
  ...
  // ============================================
  // NEW: PAYMENT CARD STATES
  // ============================================
  const [showPaymentCard, setShowPaymentCard] = useState(false);
  const [paymentCardData, setPaymentCardData] = useState(null);
  const [paymentCardType, setPaymentCardType] = useState(null);
  ...
  const loadConnectedMerchants = async () => {
    try {
      const response = await agentOrderAPI.getConnectedMerchants();
      if (response.data?.success) {
        setConnectedMerchants(response.data.data || []);
      }
    } catch (error) {
  ...
  const loadOrders = async () => {
    try {
      const response = await agentOrderAPI.getUserOrders();
  ...
  const refreshOrders = async (showToast = false) => {
    try {
      ...
      setOrders(orders);
      if (showToast)
        toast.success(`Found ${orders.length} orders!`, { id: "refresh" });
    } catch (error) {
  ...
  const loadConversationById = async (convId) => {
    try {
      const response = await agentChatAPI.getMessages(convId);
      const parsedMessages = (response.data?.data || []).map((message) => ({
        ...message,
        id: message.id || message.message_id,
        timestamp: message.created_at || message.timestamp,
        sessionId: message.session_id || message.sessionId,
        requiresAction: message.requires_action ?? message.requiresAction,
      ...
      if (parsedMessages.length) {
        ...
        const lastMsg = [...parsedMessages]
          .reverse()
          .find((m) => m.role === "agent" && m.sessionId);
    ...
    const welcomeMessage = [
      {
        ...
        role: "agent",
        content:
          "👋 Hi! I'm SabAI, your AI payment assistant. I can help you with UPI payments, bill payments, mobile recharge, food ordering, shopping, Reserve Pay limits, SabAI Gems, and Auto-Pay recurring orders. What would you like help with today?",
  ...
  // ============================================
  // NEW: PAYMENT CARD HANDLERS
  // ============================================

  const handlePaymentCardAction = (action, data) => {
    if (action === "confirm_payment" && data.requiresPin) {
      // Show PIN modal
      setPinModalData({
        bankName: data.paymentData.bankAccount.bank_name,
        bankAccountId: data.paymentData.bankAccount.id,
        amount: data.paymentData.amount,
        purpose: paymentCardType,
      });
      setPendingPaymentData(data.paymentData);
      setShowPinModal(true);
    } else if (action === "confirm_payment" && !data.requiresPin) {
      // No PIN required - confirm directly
      handleDirectConfirm(data.paymentData);
    } else if (action === "resolve_recipient") {
      // Show recipient selection
      handleRecipientSelection(data.candidates);
    }
  };
  ...
  const handleDirectConfirm = async (paymentData) => {
    setLoading(true);
    try {
      let result;
  ...
  const handleRecipientSelection = (candidates) => {
    // Show a modal or inline selection for ambiguous recipients
    ...
    <div className="agent-chat-page">
      <div className="chat-header">
        ...
        <div className="header-actions">
          ...
          <button
            className={`icon-btn ${ordersSidebarOpen ? "active" : ""}`}
            onClick={() => {
              setOrdersSidebarOpen(!ordersSidebarOpen);
              if (!ordersSidebarOpen) refreshOrders(true);
            }}
            title="My Orders"
          >
            <FaBoxOpen />
      ...
      {apiStatus === "offline" && (
        <div className="api-warning-banner">
          <FaExclamationTriangle />
          <span>Backend server offline. Check connection.</span>
        </div>
      )}
                  ...
                  <div className={`message-bubble ${message.role}`}>
                    ...
                    <div className="message-footer">
                      ...
                      {typeof message.content === "string" && (
                        ...
                        >
                          {copiedId === message.id ? <FaCheck /> : <FaCopy />}
                        </button>
                      )}
                    </div>
                  </div>
      ...
      <AnimatePresence>
        {showPinModal && pinModalData && (
          <AgentPinModal
            ...
            bankName={pinModalData.bankName}
            bankAccountId={pinModalData.bankAccountId}
            amount={pinModalData.amount}
            purpose={pinModalData.purpose}
            loading={loading}
          />
        )}
      </AnimatePresence>
