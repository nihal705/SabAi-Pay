...
class RecipientResolverService {
  ...
  async resolve(text, userId) {
    ...
    if (/\b(?:bank\s+account|account\s+number)\b/i.test(trimmed)) {
      return {
        resolved: false,
        recipient: null,
        candidates: [],
        ambiguity: false,
        message:
          "Please provide the bank account number and 11-character IFSC code, for example 14025008721991/IFSC0001234.",
      };
    }
  ...
  async resolvePhone(phone, userId) {
    ...
    return {
      ...
      isVerified: false,
      message:
        "This phone number is not in your contacts. Please verify before sending.",
    };
  }
  ...
  async resolveContactByVPA(vpa, userId) {
    ...
    const contact = contacts.find(
      (c) => c.vpa === vpa || c.vpa?.toLowerCase() === vpa.toLowerCase(),
    );
    ...
    if (parts.length === 2) {
      ...
      if (accountNumber.length >= 9 && ifscCode.length === 11) {
        return {
          ...
          isVerified: false,
          message:
            "Bank account verification required. Please confirm the account details.",
        };
      }
    }
    return null;
