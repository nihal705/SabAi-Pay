...
class AgentToolRegistry {
    ...
    buildToolRegistry() {
        return [
            ...
            // ============================================
            {
                ...
                parameters: {
                    ...
                    properties: {
                        merchant: { 
                            type: 'string', 
                            enum: ['swiggy', 'zomato', 'zepto', 'blinkit', 'amazon', 'flipkart', 'netmeds', 'pharmeasy'],
            ...
            },
            {
                ...
                description: 'List all supported merchants, optionally filtered by category',
                parameters: {
                    type: 'object',
                    properties: {
                        category: { 
            ...
            },
            {
                ...
                parameters: {
                    type: 'object',
                    properties: {
                        merchant: { type: 'string', enum: ['swiggy', 'zomato'], description: 'The merchant to search on' },
                        location: { type: 'string', description: 'City or area name' },
                        cuisine: { type: 'string', description: 'Cuisine type to filter by (optional)' }
                    },
            ...
            },
            {
                ...
                parameters: {
                    type: 'object',
                    properties: {
                        merchant: { type: 'string', enum: ['swiggy', 'zomato'], description: 'The merchant' },
            ...
            },
            {
                ...
                parameters: {
                    type: 'object',
                    properties: {
                        merchant: { type: 'string', description: 'The merchant to search in' },
                        query: { type: 'string', description: 'Search term' },
                        location: { type: 'string', description: 'City or area for location-based results' }
                    },
            ...
            },
            {
                ...
                parameters: {
                    type: 'object',
                    properties: {
                        method: { type: 'string', enum: ['upi', 'reserve_pay', 'schedule'], description: 'Payment method' },
            ...
            // ============================================
            {
                ...
                parameters: {
                    type: 'object',
                    properties: {
                        recipient: { type: 'string', description: 'Recipient identifier (UPI ID, phone number, or bank account)' },
            ...
            },
            {
                ...
                parameters: {
                    type: 'object',
                    properties: {
                        recipient: { type: 'string', description: 'Recipient identifier (UPI ID, phone number)' },
            ...
            {
                name: 'pay_bill',
                description: 'Pay a bill (electricity, mobile, broadband, gas, credit card, water)',
                parameters: {
                    type: 'object',
                    properties: {
                    ...
                    properties: {
                        payments: { 
                            ...
                            items: { 
                                type: 'object',
                                properties: {
                                    type: { type: 'string', enum: ['send_money', 'pay_bill', 'recharge_mobile'], description: 'Type of payment' },
                                    data: { type: 'object', description: 'Payment-specific data' }
                                },
                                required: ['type', 'data']
                            },
    ...
    getPaymentTools() {
        return this.tools.filter(t => 
            ['send_money', 'request_money', 'pay_bill', 'recharge_mobile', 'multi_payment'].includes(t.name)
        );
    }

    getOrderingTools() {
        return this.tools.filter(t => 
            ['check_merchant_connected', 'list_supported_merchants', 'find_restaurants_nearby', 
             'get_menu', 'search_items', 'cart_add_item', 'cart_remove_item', 'cart_update_quantity', 
             'cart_view', 'check_reserve_pay_limit', 'initiate_payment', 'get_order_status', 
             'get_user_location', 'request_location'].includes(t.name)
        );
    }
