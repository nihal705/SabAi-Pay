...
class AgentSecurityService {
  ...
  async decideSecurity(userId, transaction) {
    try {
      ...
      const sabaiPayLiteLimit = await dbService.getReserveLimit(
        userId,
        this.SABAI_PAY_LITE,
      );
      ...
      if (!sabaiPayLiteLimit || !sabaiPayLiteLimit.is_active) {
        return {
          ...
          limitRemaining: 0,
          message:
            "No SabAI Pay Lite limit set. Please set a limit in Reserve Pay.",
        };
      }

      // Calculate remaining limit
      const limitRemaining =
        (sabaiPayLiteLimit.monthly_limit || 0) -
        (sabaiPayLiteLimit.current_spent || 0);
      ...
      if (
        ...
        (transaction.type === "send_money" ||
          transaction.type === "request_money")
      ) {
        return {
          requiresPin: true,
          method: "reserve_pay",
          limitRemaining: limitRemaining,
          message:
            "First-time recipient requires PIN verification for security.",
        };
      }
    ...
    try {
      if (
        ...
        if (existingContact) {
          ...
          const hasPreviousTxn = transactions.some(
            (t) =>
        ...
        if (existingBill) {
          ...
          const hasPreviousPayment = paidBills.some(
            (pb) => pb.bill_id === existingBill.id,
          );
        ...
        const existingRecharge = recharges.find(
          (r) => r.mobile_number === transaction.mobileNumber,
        );
