...
class AgentOrderController {
  ...
  async saveSession(session) {
    try {
      const cartStr =
        typeof session.cart === "string"
          ? session.cart
          : JSON.stringify(session.cart || []);
      const merchantInfoStr = session.merchantInfo
        ? typeof session.merchantInfo === "string"
          ? session.merchantInfo
          : JSON.stringify(session.merchantInfo)
        : null;
      const preferencesStr = session.preferences
  ...
  async getUserLocationForMerchant(userId, merchant) {
    try {
      const connection = await merchantConnectionService.getMerchantConnection(
        userId,
        merchant,
      );
  ...
  async searchItemsInMerchant(merchant, searchTerm, userLocation) {
    try {
      ...
      if (merchantData?.type === "search_results" && merchantData.items) {
        ...
        merchantData?.type === "products_grid" &&
        merchantData.products
      ) {
        items = merchantData.products;
      } else if (
        merchantData?.type === "restaurant_menu" &&
        merchantData.menu
      ) {
  ...
  detectRestaurant(message) {
    ...
    const knownRestaurant = [
      ...
      "taco bell",
      "starbucks",
    ].find((name) => msg.trim() === name || msg.includes(` ${name}`));
    if (knownRestaurant) return this.fixRestaurantTypos(knownRestaurant);
    ...
    try {
      if (!userLocation?.city) {
        return res.json({
          ...
          data: {
            response:
              "📍 To find restaurants near you, please share your location first.\n\nYou can set your location in Dashboard → Connect Apps.",
      ...
      if (
        merchantData?.type === "search_results" &&
        merchantData.items?.length > 0
      ) {
      ...
      if (
        merchantData?.type === "restaurants_list" &&
        merchantData.restaurants?.length > 0
      ) {
  ...
  async processItemAddition(
    ...
    try {
      ...
      if (merchantData?.type === "products_grid" && merchantData.products) {
        ...
        console.log(
          `📦 Found ${availableProducts.length} products in products_grid`,
        );
      } else if (
        merchantData?.type === "restaurants_list" &&
        merchantData.restaurants
      ) {
        console.log(
          `📦 Found ${merchantData.restaurants.length} restaurants, fetching menus...`,
        );
        ...
        console.log(
          `📦 Collected ${availableProducts.length} menu items from restaurants`,
        );
      ...
      for (const requestedItem of items) {
        ...
        console.log(
          `🔍 Searching for: "${searchTerm}" among ${availableProducts.length} products`,
        );
        ...
        if (matchedItem) {
          console.log(
            `✅ Matched "${searchTerm}" to "${matchedItem.name}" with score ${bestScore}`,
          );
          ...
          foundItems.push({
            id:
              matchedItem.id ||
              `${merchant}_${matchedItem.name.replace(/\s/g, "_")}`,
      ...
      if (notFoundItems.length > 0 && availableProducts.length > 0) {
        console.log(
          ...
          availableProducts.slice(0, 5).map((p) => p.name),
        );
      }
      ...
      session.merchant = merchant;
      session.step =
        updatedCart.length > 0 ? "confirm_items" : "awaiting_items";
      await this.saveSession(session);
      ...
      const reserveCheck = await this.checkReservePayEligibility(
        userId,
        merchant,
        grandTotal,
      );
      ...
      return res.json({
        ...
        data: {
          response:
            "I had trouble finding those items. Please try again with exact item names from the menu.",
          requiresAction: "retry",
        },
      });
    }
  ...
  async showProductSuggestions(
    ...
    try {
      ...
      if (
        merchantData?.type === "products_grid" &&
        merchantData.products?.length > 0
      ) {
        const filteredProducts =
          orderPreferenceParser.applyPreferencesToResults(
            merchantData,
            preferences,
            merchantData.products,
          );
  ...
  // ============================================
  async getSuggestedItemsFromNaturalLanguage(
    message,
    merchant,
    userLocation = null,
  ) {
  ...
  getDefaultItemImage(itemName) {
    ...
    if (lowerName.includes("biryani"))
      return "/images/items/chicken-biryani.png";
    if (lowerName.includes("paneer"))
      return "/images/items/paneer-butter-masala.png";
    if (lowerName.includes("chicken"))
      return "/images/items/butter-chicken.png";
  ...
  async checkReservePayEligibility(userId, merchant, amount) {
    try {
      ...
      const limit =
        merchantLimit ||
        (await dbService.getReserveLimit(userId, "sabai-pay-lite"));
  ...
  async processOrder(req, res) {
    try {
      ...
      try {
        ...
        console.log(
          `🧠 ML Intent: ${mlResult.intent} (confidence: ${mlResult.confidence})`,
        );
        ...
        console.error(
          "ML service error, falling back to rule-based:",
          mlError.message,
        );
      ...
      if (
        ...
        ["confirm_items", "payment_selection"].includes(session.step) &&
        /\b(confirm|proceed|continue|show\s*(my\s*)?cart|where.*confirm|payment\s*option)/i.test(
          message,
        )
      ) {
        const reserveCheck = await this.checkReservePayEligibility(
        ...
        const orderSummary = this.formatOrderSummary(
          ...
          session.merchantInfo,
          reserveCheck,
          this.calculateSabaiGems(session.total),
          session.merchant,
        );
        return res.json({
          success: true,
      ...
      console.log(
        `🎯 Final Intent: ${finalIntent} (ML confidence: ${session.mlConfidence || 0}, using ML: ${useML})`,
      );

      // Continue an active order even when ML labels a short item reply
      // such as "2 Chicken Biryani" as a general message.
      if (
        session.merchant &&
        ["awaiting_items", "selecting_items"].includes(session.step)
      ) {
        let continuationItems = (session.mlEntities?.items || []).map(
          (item, index) => ({
            name: item.name,
            quantity:
              parseInt(session.mlEntities?.quantities?.[index]?.value, 10) || 1,
          }),
        );
        if (!continuationItems.length) {
          continuationItems = intentClassifier.extractItemsFromMessage(message);
        }
        if (continuationItems.length > 0) {
          const userLocation =
            reqUserLocation ||
            (await this.getUserLocationForMerchant(userId, session.merchant));
          return await this.processItemAddition(
            req,
            res,
            session,
            session.merchant,
        ...
        if (this.hasRestaurantName(message)) {
          const userLocation =
            reqUserLocation ||
            (await this.getUserLocationForMerchant(userId, session.merchant));
          const restaurantName =
            this.detectRestaurant(message) ||
            this.fixRestaurantTypos(message.trim());
          return await this.showRestaurantMenu(
            req,
            res,
            session,
            session.merchant,
      ...
      if (
        finalIntent === "order" ||
        finalIntent === "order_new" ||
        finalIntent === "order_ask_items"
      ) {
        let merchant =
          session.merchant ||
        ...
        if (!merchant) {
          return res.json({
            ...
            data: {
              response:
                "Which platform would you like to order from? (Swiggy, Zomato, Zepto, Amazon, Flipkart, etc.)",
              requiresAction: "clarify_merchant",
            },
          });
        }

        const isConnected = await merchantConnectionService.isConnected(
          userId,
          merchant,
        );
        if (!isConnected) {
        ...
        if (!userLocation || !userLocation.city) {
          const savedLocation = await this.getUserLocationForMerchant(
            userId,
            merchant,
          );
          if (savedLocation) {
            ...
            console.log(
              `📍 Using saved location for ${merchant}: ${userLocation.city}`,
            );
          }
        }
        ...
        // ============================================
        const explicitMenuRequest =
          message.toLowerCase().includes("show me menu") ||
          message.toLowerCase().includes("menu from");
        const hasRestaurant = this.hasRestaurantName(message);
        const isShowMenuRequest =
          explicitMenuRequest ||
          (hasRestaurant && message.toLowerCase().includes("from"));
        ...
        const isPlatformOnlyRequest = platformOnlyPatterns.some((pattern) =>
        ...
        if (isShowMenuRequest) {
          ...
          if (!restaurantName) {
            return res.json({
              ...
              data: {
                response:
                  "Which restaurant would you like to see the menu from? Please specify the restaurant name.",
        ...
        if (session.mlEntities?.items && session.mlEntities.items.length > 0) {
          extractedItems = session.mlEntities.items.map((item) => ({
            ...
            quantity:
              parseInt(
                session.mlEntities.quantities?.find((q) => q.name === item.name)
                  ?.value,
              ) || 1,
          }));
      ...
      if (
        finalIntent === "hybrid" ||
        finalIntent === "hybrid_situation_to_order"
      ) {
        ...
        if (
          finalSubType === "situation_to_order" ||
          finalIntent === "hybrid_situation_to_order"
        ) {
          const situationAnalysis =
            await situationToOrderService.analyzeSituation(message);
          ...
          if (situationAnalysis && situationAnalysis.confidence > 0.3) {
            ...
            const suggestions =
              await situationToOrderService.generateInteractiveSuggestions(
                merchant,
                situationAnalysis,
                userLocation,
                merchantDataService,
      ...
      if (finalIntent === "order_ask_platform") {
        return res.json({
          ...
          data: {
            response:
              "Which platform would you like to order from?\n\nAvailable: Swiggy, Zomato, Zepto, Blinkit, Amazon, Flipkart, NetMeds, PharmEasy",
            requiresAction: "clarify_merchant",
          },
        });
      }
      ...
      if (finalIntent === "general_comparison") {
        return res.json({
          ...
          data: {
            response:
              "🔍 I can help you compare prices and delivery times across platforms. Which platforms would you like to compare? (e.g., Swiggy vs Zomato)",
      ...
      if (
        finalIntent === "payment_immediate" ||
        finalIntent === "payment_auto_pay"
      ) {
        if (session.cart.length === 0) {
          return res.json({
            success: true,
      ...
      return res.json({
        ...
        data: {
          response:
            "How can I help you today? You can ask me to order food, groceries, medicines, or just chat with me!",
          requiresAction: "general_chat",
        },
      });
    } catch (error) {
  ...
  async selectItems(req, res) {
    try {
      ...
      if (
        ...
        session.total = grandTotal;
        session.step =
          updatedCart.length > 0 ? "payment_selection" : "awaiting_items";
        await this.saveSession(session);
      ...
      if (msg === "2") {
        return res.json({
          ...
          data: {
            response:
              "Please type the correct names for the items you want to order. For example: '2 Chicken Biryani, 1 Butter Chicken'",
            requiresAction: "specify_items",
            sessionId: session.id,
          },
        });
      ...
      if (msg.length > 2 && !["1", "2", "3"].includes(msg)) {
        return this.processItemAddition(
          req,
          res,
          session,
          session.merchant,
    ...
    try {
      ...
      const tracking = [
        ...
        },
        {
          ...
          label: "Preparing",
          completed: false,
          estimatedTime: deliveryTime.toLocaleTimeString(),
        },
        {
      ...
      return res.json({
        ...
        data: {
          response:
            "Payment was successful but order confirmation failed. Please check My Orders.",
          requiresAction: "retry",
        },
      });
    }
  ...
  async processReservePayment(req, res) {
    try {
      ...
      const sabaiGems = this.calculateSabaiGems(roundedTotal);
      await this.updateUserReserveLimit(
        userId,
        reserveCheck.merchant || merchant,
        roundedTotal,
      );
  ...
  async getUserOrders(req, res) {
    try {
      ...
      allOrders.sort(
        (a, b) =>
          new Date(b.createdAt || b.scheduledTime) -
          new Date(a.createdAt || a.scheduledTime),
      );
  ...
  async getOrder(req, res) {
    try {
      ...
      const created = new Date(
        order.createdAt || order.scheduledTime,
      ).getTime();
  ...
  async checkReservePayLimit(req, res) {
    try {
      ...
      const userId = req.user?.id ? String(req.user.id) : "4";
      const result = await this.checkReservePayEligibility(
        userId,
        merchant,
        amount,
      );
  ...
  async checkMerchantConnection(req, res) {
    try {
      ...
      return res.json({
        success: true,
        data: { connected: isConnected, merchantId },
      });
    } catch (error) {
  ...
  async getConnectedMerchants(req, res) {
    try {
      const userId = req.user?.id ? String(req.user.id) : "4";
      const connections =
        await merchantConnectionService.getConnectedMerchants(userId);
  ...
  async confirmAutoPaySetup(req, res) {
    try {
      const { sessionId, schedule, dayOfMonth, bankAccountId, scheduledTime } =
        req.body;
      const userId = req.user?.id ? String(req.user.id) : "4";
      ...
      if (scheduledTime) {
        ...
        await this.saveSession(session);
        return await this.processReservePayment(
          { body: { sessionId }, user: { id: userId } },
          res,
        );
      }
      ...
      if (!selectedBank) {
        return res.json({
          ...
          data: {
            response:
              "Selected bank account not found. Please add a bank account in Settings first.",
      ...
      return res.json({
        ...
        data: {
          response: {
            ...
            nextPayment: nextDate.toLocaleDateString("en-IN", {
              day: "numeric",
              month: "long",
              year: "numeric",
            }),
