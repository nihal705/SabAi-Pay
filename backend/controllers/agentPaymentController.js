...
class AgentPaymentController {
  ...
  async confirmSendMoney(userId, paymentData, pin = null) {
    try {
      ...
      if (paymentData.method !== "reserve_pay") {
        const account = await dbService.getBankAccountById(
          fromAccountId,
          userId,
        );
      ...
      if (paymentData.method === "reserve_pay") {
        await dbService.updateReserveLimitSpent(
          userId,
          "sabai-pay-lite",
          amount,
        );
      ...
      await dbService.createTransaction({
        ...
        description:
          paymentData.note ||
          `Payment to ${paymentData.recipient.displayName || paymentData.recipient.vpa}`,
        receiver_vpa: paymentData.recipient.vpa,
        receiver_name:
          paymentData.recipient.displayName || paymentData.recipient.name,
        bank_name: paymentData.bankAccount.bank_name,
        bank_account_id: fromAccountId,
        payment_method_display:
          paymentData.method === "reserve_pay"
            ? "SabAI Pay Lite"
            : "Bank Transfer",
      ...
      await dbService.createOrUpdateContact(
        userId,
        paymentData.recipient.displayName ||
      ...
      return {
        ...
        amount: amount,
        recipient:
          paymentData.recipient.displayName || paymentData.recipient.vpa,
        method:
          paymentData.method === "reserve_pay"
            ? "SabAI Pay Lite"
            : "Bank Transfer",
  ...
  async payBill(userId, billType, provider, customerId, amount) {
    try {
      ...
      if (!biller) {
        return {
          error: `Bill provider "${provider}" not supported. Supported providers: ${billerCatalogService
            .getAllBillers()
            .map((b) => b.name)
            .join(", ")}`,
          status: "failed",
      ...
      return {
        ...
        requiresAction: "confirm_payment",
        paymentData: paymentData,
        message: this.buildBillPaymentMessage(
          biller,
          customerId,
          amount,
          security,
  ...
  async confirmBillPayment(userId, paymentData, pin = null) {
    try {
      ...
      await dbService.markBillAsPaid(billId, userId, {
        payment_method:
          paymentData.method === "reserve_pay" ? "reserve_pay" : "bank",
        payment_breakdown: {
  ...
  async rechargeMobile(
    ...
    try {
      if (!/^[6-9]\d{9}$/.test(mobileNumber)) {
        return {
          error:
            "Invalid mobile number. Please enter a 10-digit Indian number.",
          status: "failed",
        };
      }

      const detectedOperator = rechargePlanService.detectOperator(mobileNumber);
      const requestedOperatorId = String(requestedOperator || "")
        .toLowerCase()
        .replace("vodafone", "vi");
      const operator = requestedOperatorId
        ? rechargePlanService.getOperatorById(requestedOperatorId)
        : detectedOperator;
      if (!operator) {
        return {
          error:
            "Could not detect operator for this mobile number. Please verify the number.",
          status: "failed",
        };
      }
      ...
      if (plan) {
        selectedPlan = plans.find(
          (p) =>
            p.name.toLowerCase().includes(plan.toLowerCase()) || p.id === plan,
        );
      }
      if (!selectedPlan && amount) {
        selectedPlan = rechargePlanService.getRecommendedPlan(
          operator.id,
          amount,
        );
      }
      ...
      return {
        ...
        requiresAction: "confirm_payment",
        paymentData: paymentData,
        message: this.buildRechargeMessage(
          mobileNumber,
          operator,
          amount,
          selectedPlan,
  ...
  async confirmRecharge(userId, paymentData, pin = null) {
    try {
      ...
      const cashbackEarned =
        paymentData.method === "reserve_pay"
          ? Math.min(Math.floor(amount * 0.05), 100)
          : 0;
    ...
    try {
      ...
      for (const payment of payments) {
        ...
        if (payment.type === "send_money") {
          ...
          result = await this.payBill(
            userId,
            payment.data.billType,
            payment.data.provider,
            payment.data.customerId,
            payment.data.amount,
          );
        } else if (payment.type === "recharge_mobile") {
          result = await this.rechargeMobile(
            userId,
            payment.data.mobileNumber,
            payment.data.amount,
            payment.data.plan,
          );
        } else {
